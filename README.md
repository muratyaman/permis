...
```
https://[api-gateway]/billing-api/v1/invoices*
```
...
```
POST https://[idp]/v1/users
```

## C. Consumers

API Consumer registers

* Payment may be required
* We may need to **approve** this or **activate** the account

```
POST https://[oauth2]/consumers
```

including name (e.g. company name), user_id (refers to IdP)
...
API Client initiates a Consent request - redirect browser:

```
GET https://[oauth2]/authorize?response_type=RESPONSE-TYPE&client_id=CLIENT-ID&scope=SCOPE&state=CLIENT-CROSS-REF&redirect_uri=REDIRECT-URI
```

* `RESPONSE-TYPE` can be `code` (default) or `token`
* `CLIENT-ID` must be ID of an existing client
* `SCOPE` must be a list of scopes using existing scope IDs - separated by spaces
...
For example, let's use `https://app.example.com` as `REDIRECT-URI`. It must not include any **parameters**.
...
```
GET https://[oauth2]/consents/:id
GET https://[oauth2]/clients/:id
GET https://[oauth2]/consumers/:id
```
...
```
PATCH https://[oauth2]/consents/:id
```
...
```
DELETE https://[oauth2]/consents/:id
```
...
```
POST https://[oauth2]/authorize?response_type=RESPONSE-TYPE&client_id=CLIENT-ID&scope=SCOPE&state=CLIENT-CROSS-REF&redirect_uri=REDIRECT-URI&consent_id=CONSENT-ID&allowed=1
```
